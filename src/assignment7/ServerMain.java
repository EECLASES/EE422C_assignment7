...
public class ServerMain  {	
	...
	public void parse2(ClientObserver person, String message) {
			...
			synchronized(people) {
				if(people.containsKey(username)) {
					if(person.getPassword().equals(pass)) {
						...
						person.println(person.getPersonInfo() + "Incorrect password, try again " + person.getName() );
					}
					person.flush();
				}else if(!people.containsKey(username)) {
					
					
					person.println(person.getPersonInfo() + "No User found with this username " + person.getName() );
					person.flush();
